...
for i in range(0, len(grid)):
    for j in range(0, len(block)):
        for k in range(0, len(batch)):
            filename = mpscriptdir + "grid"+str(grid[i])+"_block"+str(block[j])+"_batch"+str(batch[k])+".sub"
            f = open(filename, "w+")
            f.write('#!/bin/bash\n')
            f.write('#SBATCH -J shallow\n')
            f.write('#SBATCH -o '+ mpresultdir + 'grid'+str(grid[i])+'_block'+str(block[j])+'_batch'+str(batch[k])+'.out\n')
            f.write('#SBATCH -e '+ mpresultdir + 'grid'+str(grid[i])+'_block'+str(block[j])+'_batch'+str(batch[k])+'.err\n')
            f.write('#SBATCH --nodes=1 \n')
            f.write('#SBATCH --ntasks=1 \n')
            f.write('#SBATCH --tasks-per-node=1 \n')
