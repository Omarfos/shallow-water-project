    ...
    for j in range(0, len(block)):
        if grid[i] > block[j]:
            ...
            for k in range(0, len(batch)):
                filename = mpmpiscriptdir + "grid"+str(grid[i])+"_block"+str(block[j])+"_batch"+str(batch[k])+".sub"
                f = open(filename, "w+")
                f.write('#!/bin/bash\n')
                f.write('#SBATCH -J shallow\n')
                f.write('#SBATCH -o '+ mpmpiresultdir + 'grid'+str(grid[i])+'_block'+str(block[j])+'_batch'+str(batch[k])+'.out\n')
                f.write('#SBATCH -e '+ mpmpiresultdir + 'grid'+str(grid[i])+'_block'+str(block[j])+'_batch'+str(batch[k])+'.err\n')
